...
def commit_to_github(files: Dict[str, bytes], face_id: str) -> Dict[str, Optional[str]]:
    if not GITHUB_ENABLED:
...
async def add_face(
    ...
    if existing:
        return FaceAddResponse(
            ok=True,
            face_id=existing["id"],
            image_path=existing.get("local_path", ""),
            image_url=existing.get("github_url") or existing.get("server_url") or "",
            ...
            message="Face already ingested.",
        )

    record = ingest_file(content, image.content_type, metadata_obj)
    ...
    return FaceAddResponse(
        ...
        image_path=str(record["image_path"].relative_to(DATA_DIR)),
        image_url=image_url,
...
async def sync_faces(request: Request, images: Optional[List[UploadFile]] = File(None), metadata: Optional[str] = Form(None)):
    ...
    if files_to_commit and GITHUB_ENABLED:
        ...
        try:
            commit_info = commit_to_github(files_to_commit, created_face_ids[0])
            for face_id in created_face_ids:
                github_url = raw_github_url(
                    f"data/faces/images/{image_name_map.get(face_id, f'{face_id}.jpg')}",
                    branch=commit_info["branch"] or GITHUB_BRANCH,
                )
                update_index_commit_info(
                    face_id,
